...
BEGIN
	...
	AND TABLE_NAME = 'ComEx_CurrencyInternalTrade'  AND COLUMN_NAME = 'AgreedTerm';
	IF l_rows = 1 THEN
		SELECT COUNT(1) INTO l_rows FROM ALL_TAB_COLUMNS WHERE OWNER = SYS_CONTEXT ('USERENV', 'CURRENT_SCHEMA') 
		AND TABLE_NAME = 'ComEx_CurrencyInternalTrade' AND COLUMN_NAME = 'Term';
		IF l_rows = 1 THEN
			SELECT COUNT(1) INTO l_rows FROM "ComEx_CurrencyInternalTrade" 
			WHERE "Term" != (SELECT "Oid" FROM "ComEx_Term" WHERE "MinimumDays" = 0 AND "MaximumDays" = 0);
			IF l_rows = 0 THEN
				UPDATE "ComEx_CurrencyInternalTrade" SET "AgreedTerm" = 
				...
				EXECUTE IMMEDIATE 'ALTER TABLE "ComEx_CurrencyInternalTrade" DROP CONSTRAINT "' || l_name_FK || '"';
				EXECUTE IMMEDIATE 'ALTER TABLE "ComEx_CurrencyInternalTrade" DROP COLUMN "Term"';
			ELSE
